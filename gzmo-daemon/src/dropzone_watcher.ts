/**
 * dropzone_watcher.ts — Watch GZMO/Dropzone/ for arbitrary files.
 *
...
/**
 * Handle one stable file path under GZMO/Dropzone/ (not a directory, not under _processed/_failed).
 */
export async function handleDropzoneFile(absPath: string, deps: DropzoneWatcherDeps): Promise<void> {
  ...
  if (bn.startsWith(".") || bn === "_processed" || bn === "_failed" || bn === "files" || bn === "_tmp") {
    return;
  }
  ...
  try {
    ...
    if (bn.toLowerCase().endsWith(".md")) {
      ...
      const slug = sanitizeDropzoneBaseName(bn);
    ...
    if (needOuterBytesForDedup && outerFileBuf) {
      try {
        ...
        if (prior) {
          ...
          const stubSlugDup = sanitizeDropzoneBaseName(bn);
    ...
    const stubSlug = sanitizeDropzoneBaseName(bn);
...
export async function scanDropzoneOnBoot(deps: DropzoneWatcherDeps): Promise<void> {
  ...
  let names: string[];
  try {
    names = await readdir(dropDir);
  } catch {
    return;
  }
  for (const name of names) {
    if (name.startsWith(".") || name === "_processed" || name === "_failed" || name === "files" || name === "_tmp")
      continue;
    const abs = join(dropDir, name);
    let st;
    try {
      st = await lstat(abs);
    } catch {
