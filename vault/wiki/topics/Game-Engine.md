---
title: Game Engine
type: topic
tags:
  - rust
  - game-engine
  - polyhedral
  - dynamics
  - programming
sources: 2
created: 'Mon Apr 13 2026 02:00:00 GMT+0200 (Central European Summer Time)'
updated: '2026-04-24'
---
# Game Engine

Rust-based game engine development with a focus on deterministic polyhedral dynamics and trading card game architecture.

## Projects

### Polyhedral Dynamics Engine
- Deterministic physics simulation in Rust
- Polyhedral collision and interaction models

### Trading Card Game Architecture
- Game design and mechanics
- Architecture of engagement and immersion

## Related

- [[Linux-Workstation]] — Development platform

## Sources

- `raw/notebooklm/Deterministic_Polyhedral_Dynamics_in_Rust_Game_Eng*`
- `raw/notebooklm/The_Architecture_of_a_Trading_Card_Game*`
- `raw/notebooklm/The_Architecture_of_Engagement*`
- `raw/notebooklm/The_Architecture_of_Immersion*`

- [[source-4b77aeee-ad27-47ea-94b3-6b6938062915-artifacts-implementation-plan]]
