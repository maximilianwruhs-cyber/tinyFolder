...
export default function gzmoTinyFolderExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "For ad-hoc files without hand-authored frontmatter, the user can drop files or whole folder trees under VAULT_PATH/GZMO/Dropzone/ (recursive ingest; root `_processed/`, `_failed/`, `files/`, `_tmp/` only are daemon-reserved). Supported conversions (when enabled) include pdf, docx, html, txt, csv, json; optional ZIP unpack (GZMO_DROPZONE_ZIP) picks the first matching inner file; dedup uses GZMO/.gzmo_dropzone_index.json.",
    ],
